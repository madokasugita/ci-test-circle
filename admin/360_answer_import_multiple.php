...
/****************************************************************************************************************************/
define('PAGE_TITLE', '	評価データインポート(複数設問対応)');
/****************************************************************************************************************************/
...
class ThisImportModel extends ImportModel360
{
    ...
    public function importLine($line_no, $data) //override
    {
        ...
        foreach ($subevents as $subevent) {
            ...
            } elseif ($subevent['type2']=='c') {
                $emptyFlag = true;
                $ch = explode(',', $subevent['choice']);
    ...
    public function setChoiceHeaders()
    {
        $this->choiceSeidCount = array();
        foreach ($this->header as $header) {
            if (preg_match('/-[0-9]+$/', $header)) {
                $seid = preg_replace('/-[0-9]+$/', '', $header);
                $this->choiceSeidCount[$seid] = (is_void($this->choiceSeidCount[$seid])) ? 1 : (int) $this->choiceSeidCount[$seid] + 1;
            }
        }
    }
    ...
    public function getErrors($line_no, $data) //override
    {
        ...
        */
            ...
            } elseif ($subevent['type2']=='c' && is_good($this->choiceSeidCount[$subevent['seid']])) {
                $choices = explode(',',$subevent['choice']);
                if ($this->choiceSeidCount[$subevent['seid']] != count($choices)) {
                    $error[] = $line_no."行目 {$r}列目:({$array['uid']}<-{$array['uid_t']}) 複数設問数が異なっています。複数設問数は".count($choices)."です。";
                    continue;
                }
...
class ThisImporter extends Importer360
{
    ...
    public function downloadSubevents()
    {
        ...
        $where = "WHERE ".$evid."type2 IN ('r','p','t','c') ORDER BY seid,evid";
        ...
        foreach ($_csv as $v) {
            ...
            if ($v['type2'] == 'c') {
                $i = 0;
                foreach (explode(',', $v['choice']) as $seid) {
                    $i++;
                    $csv[$v['evid']][] = $v['seid'].'-'.$i;
                }
            } else {
