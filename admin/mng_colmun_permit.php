...
if ($_POST["main"] == "更新") { //更新
    Set_Musr($_POST, "permit_column", $_POST["fMUID"]);
}
...
//デフォルト
$html .= '<br>';
$html .= '<table style="margin-left:30px"border=0 cellspacing=0 cellpadding=2>';
$html .= '<tr>';
$html .= '<td>&nbsp;</td>';
$c = 0;
foreach ($GLOBALS['aryColumn'] as $menu) {
    if ($lastcategory <> $menu[0])
        ++ $c;
...
foreach ($array as $ar) {
    ...
    $html .= '<tr>';
    $html .= '<td>' . html_escape($ar["div"]) . ' ' . html_escape($ar["name"]) . '</td>';
    if ($ar["muid"] == $_POST["fMUID"]) { //編集対象
        foreach ($GLOBALS['aryColumn'] as $menu) {
            $html .= '<td><input type="checkbox" name="ok[]" value="' . $menu[1] . '" ' .
             (strstr($ar["permitted_column"], $menu[1]) == false ? "" : " checked") .
            '></td>';
        }
    } else {
        $c = 0;
        ...
        foreach ($GLOBALS['aryColumn'] as $menu) {
            if ($lastcategory <> $menu[0])
                ++ $c;
            $html .= '<td' . (($c % 2) == 0 ? "" : " bgcolor=#f2f2f2") . '>' . (strstr($ar["permitted_column"], $menu[1]) == false ? "×" : "○") . '</td>';
            $lastcategory = $menu[0];
        }
    }
    //ボタン
    if ($ar["muid"] == $_POST["fMUID"]) { //編集対象
        $html .= '<td><input type="submit" name="main" value="更新"></td>';
    } else {
...
//$html = RD::getSubject("ページ別権限管理").$html;

$objHtml = & new MreAdminHtml("ユーザ情報権限管理");
