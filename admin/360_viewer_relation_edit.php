...
function main()
{
    ...
    $body = $sl->show(array ('op' => 'search', 'div1' => 'default', 'div2' => 'default', 'div3' => 'default', 'name' => '', 'uid' => '', 'email' => '', 'user_type' => '', 'get_limit' => '50','sort_' => '5'));

    $body = str_replace('%%%%PHP_SELF%%%%', $s->getLink().'&sort_='.(int) $_REQUEST['sort'].'&desc='.(int) $_REQUEST['desc'], $body);
...
class ThisCondTableView extends CondTableView
{
    public function getBox($row, $hidden, $action)
    {
        ...
        $users = FDB::getAssoc("select b.* from {$T_USER_RELATION} a left join {$T_USER_MST} b on user_type = {$GDF->get('VIEWER_USER_TYPE')} and a.uid_b = b.uid where user_type = {$GDF->get('VIEWER_USER_TYPE')} and uid_a = '{$TARGET_USR_UID}';") ;
...
class SortAdapter extends SortTableAdapter
{
    public function getResult($where)
    {
        ...
        $TARGET_USR_UID = TARGET_USR_UID;
        $sql =<<<SQL
select * from {$T_USER_MST} a left join {$T_USER_RELATION} b on b.user_type = {$GDF->get('VIEWER_USER_TYPE')} and a.uid = b.uid_b and b.uid_a = '{$TARGET_USR_UID}' {$where};
SQL;
    ...
    public function getCount($where)
    {
        ...
        $TARGET_USR_UID = TARGET_USR_UID;
        $sql =<<<SQL
select count(*) as count from {$T_USER_MST} a left join {$T_USER_RELATION} b on  b.user_type = {$GDF->get('VIEWER_USER_TYPE')} and a.uid = b.uid_b and b.uid_a = '{$TARGET_USR_UID}' {$where};
SQL;
    ...
    public function getColumnValue($data, $key)
    {
        ...
        switch ($key) {
            case 'user_type' :
                if($val != VIEWER_USER_TYPE)

                    return "-";
                return '<b>'.getUserTypeNameById($val).'</b>';

            case 'button':
                $data['user_type'] = ($data['user_type']==VIEWER_USER_TYPE) ? $data['user_type'] : 0;
                $disabled[$data['user_type']] = ' disabled';

                foreach ($_360_user_type as $k => $v) {
                    if($k!=VIEWER_USER_TYPE)
                        continue;
...
class ThisCond extends CondTableAdapter
{
    ...
    public function getColumnForms($def, $key)
    {
        ...
        switch ($key) {
            case "user_type":
                $array = array(''=>'指定しない');
                $array[VIEWER_USER_TYPE] = $_360_user_type[VIEWER_USER_TYPE];
...
function relationEdit($user)
{
    ...
    $where = 'where user_type = '.VIEWER_USER_TYPE.' and uid_a = '.FDB::escape($user['uid']).' and uid_b = '.FDB::escape($_POST['target_uid']);
