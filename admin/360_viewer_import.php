...
class ThisImportModel extends ImportModel360
{
    public function importLine($line_no, $data) //override
    {
        ...
        $array['user_type'] = VIEWER_USER_TYPE;

        $rs = $this->relation_dalete($array);
        if ($data[2]!="1" && $data[2]!="削除") {
    ...
    {
        return FDB :: delete(T_USER_RELATION, 'where user_type = '.VIEWER_USER_TYPE.' and uid_a = ' . FDB :: escape($data['uid_a']).' and  uid_b = ' . FDB :: escape($data['uid_b']));
    }
    ...
    public function getErrors($line_no, $data) //override
    {
        ...
        elseif($this->users[$array['uid_a']]=='-1')
            $error[] = "{$line_no}行目:指定した対象者は対象者フラグが0です。( <b>{$o_uida}</b> )";

        if (FCheck :: isBlank($array['uid_b']))
            $error[] = "{$line_no}行目:参照者は必須です。";
        elseif(!$this->users[$array['uid_b']])
            $error[] = "{$line_no}行目:指定した参照者は登録されていません。( <b>{$o_uidb}</b> )";

        if (!FCheck :: isBlank($array['uid_a']) && !FCheck :: isBlank($array['uid_b']) && $array['uid_a']==$array['uid_b'])
            $error[] = "{$line_no}行目:対象者と参照者が同一です。( <b>{$o_uida}</b>, <b>{$o_uidb}</b> )";

        //if ($array['user_type'] && !getUserTypeNameById($array['user_type']))
        //	$error[] = "{$line_no}行目:回答者タイプの値が不正です。( <b>{$array['user_type']}</b> )";
        return $error;
    }
    /**
...
class ThisImportDesign extends ImportDesign360
{
    ...
    {
        return '<div style="margin-top:30px"><a href="https://s3-ap-southeast-1.amazonaws.com/smart-review/samplecsv/viewer.csv">サンプルCSVダウンロード</a></div>';
    }
