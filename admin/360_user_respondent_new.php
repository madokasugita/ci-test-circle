...
class ThisAdapter extends MultiDataEditAdapter
{
    ...
    public function insert($data)
    {
        ...
        $data['lang_type'] = ($data['lang_type'])? $data['lang_type']:0;
        $array = array();
...
//	}
    ...
    public function getFormCallback($data, $col)
    {
        ...
        switch ($col) {
            ...
            default :
                return FForm :: text($col."[".$now."]", $value,null,'style="width:170px"');
        }
...
$(function () {
    $("#add_row").click(function () {
        cols = $("[name^='name']");
        len = cols.length;
        tr = cols.eq(len-1).parent().parent();
        tr.clone(true).insertAfter(tr).children().each(function () {
            $(this).children()
                .val("")
                .attr('id', $(this).children().attr('id').replace(len, len+1))
                .attr('name', $(this).children().attr('name').replace(len, len+1))
                ;
        });
    });
});
...
<!--
<br/><br/>
<input type="button" value="####continue_register####" style="width:250px;height:25px;" onclick="location.href='360_user_respondent_new_u.php?{$SID}&mode=new'">
-->
