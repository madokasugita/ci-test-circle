...
function main()
{
    ...
    $objHtml = new MreAdminHtml("対象者選定検索");
    $objHtml->setExFix();
    $DIR_IMG = DIR_IMG;
...
class ThisSortView extends ResearchSortTableView
{
    public function __construct()
    {
        parent::__construct();
        $width = array();
        $width = array_merge($width,getColmunWidth('target_relation_search'));
...
//		http://192.168.11.2/imai/360degree/56G/360_target_relation_search.php#page?op=search&sheet_type=all&div1=default&div2=default&div3=default&name=&name_=&uid=&email=&countall=&count1=&count2=&count3=&get_limit=50&SID=02059cab01de6caffcd1cc05753c0180&offset=50
//		$link =  sprintf('%s%s%s',substr($link,$start = 0 ,$pos = strpos($link,"?")),"#page",substr($link, $pos - $start, strlen($link) - $pos));
...
class SortAdapter extends SortTableAdapter
{
    public function getResult($where)
    {
        ...
        foreach (range(1, INPUTER_COUNT) as $t) {
            $each_count[] = <<<__SQL__
(select count(*) as count from {$T_USER_RELATION} where uid_b = uid and user_type= {$t}) as count{$t}
__SQL__;
        }
        ...
        $sql =<<<SQL
select *,
(select count(*) as count from {$T_USER_RELATION} where uid_b = uid and user_type <= {$INPUTER_COUNT}) as countall,
{$each_count}
...
SQL;
    ...
    {
        return date('Ymd').'対象者'.DATA_FILE_EXTENTION;
    }
    public function getColumns()
    {
        ...
        $array = array();
        $array['button'] = " ";
        $array = array_merge($array,getColmunLabel('target_relation_search'));
        $array['countall'] = "対象者<br>合計数";
...
function getHtmlUserEditButton($serial_no)
{
    ...
    return<<<HTML
<form action="360_target_relation_view.php?{$SID}" method="post" target="_blank" style="display:inline;margin:0px;">
